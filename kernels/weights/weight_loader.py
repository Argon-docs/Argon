...
def load_and_pack(model_dir: str) -> Dict[str, Any]:
    ...
    # Handle HuggingFace model identifiers (e.g., "meta-llama/Llama-2-7b-hf")
    # Check if it's a HuggingFace model identifier (contains '/' and doesn't exist as a local path)
    if "/" in model_dir and not os.path.isdir(model_dir):
        try:
            from huggingface_hub import snapshot_download
            # Download or get cached model path (download safetensors files)
            actual_model_dir = snapshot_download(
                repo_id=model_dir,
            ...
            raise RuntimeError(
                f"huggingface_hub is required to load models from HuggingFace Hub. "
                f"Install it with: pip install huggingface_hub"
            )
        except Exception as e:
            raise RuntimeError(
                f"Could not download/resolve HuggingFace model '{model_dir}'. "
                f"Error: {e}. Make sure you have access to the model and huggingface_hub is installed."
            )
    ...
    if not shard_paths:
        raise RuntimeError(
            ...
            f"Make sure the model is downloaded and contains .safetensors files. "
            f"If using a HuggingFace model identifier, ensure you have access to the model."
        )
